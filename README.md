...
For using other server than the one defined on index 0 set context value `groupdocs_translation_cloud.ContextServerIndex` of type `int`.
...
Templated server URL is formatted using default variables from configuration or from context value `groupdocs_translation_cloud.ContextServerVariables` of type `map[string]string`.
...
Similar rules for overriding default operation server index and variables applies by using `groupdocs_translation_cloud.ContextOperationServerIndices` and `groupdocs_translation_cloud.ContextOperationServerVariables` context maps.
...
```go
auth := context.WithValue(context.Background(), groupdocs_translation_cloud.ContextAccessToken, "ACCESSTOKENSTRING")
...
auth := context.WithValue(oauth2.NoContext, groupdocs_translation_cloud.ContextOAuth2, tokenSource)
r, err := client.Service.Operation(auth, args)
```
